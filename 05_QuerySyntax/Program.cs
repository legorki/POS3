...
namespace QuerySyntax
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            IEnumerable<Test> result1 = from test in data.Test
                                        where test.TE_Teacher == "ZUM" && test.TE_Class == "4EHIF"
                                        select test;
            ...
            IEnumerable<Lesson> result2 = from lesson in data.Lesson
                                          where lesson.L_Class == "3BHIF" && lesson.L_Teacher == "FZ"
                                          orderby lesson.L_Day, lesson.L_Hour
                                          select lesson;
            ...
            var result3 = from lesson in result2
                          select new
                          {
                              Subject = lesson.L_Subject,
                              Room = lesson.L_Room,
                              Day = lesson.L_Day,
                              Hour = lesson.L_Hour
                          };
            ...
            var result4 = (from test in data.Test
                           where test.TE_Subject == "NW2"
                           select new
                           {
                               TeacherId = test.TE_TeacherNavigation.T_ID,
                               TeacherName = $"{test.TE_TeacherNavigation.T_Lastname} {test.TE_TeacherNavigation.T_Firstname}"
                           }).Distinct();
            ...
            object result5 = (from c in data.Test
                              where c.TE_LessonNavigation.Lessons.Any(l => l.L_Hour > 7)
                              where c.TE_ClassNavigation.C_ClassTeacherNavigation.T_ID == "SZ"
                              select c.TE_Subject)
                .Distinct()
          ...
          .ToList();
            ...
            var result6 = data.Test
    .Where(t => data.Schoolclass
        .Where(c => c.C_ID == t.TE_Class && c.C_ClassTeacher == "SZ")
        .Any())
    .Where(t => t.TE_Date.DayOfWeek == DayOfWeek.Monday)
    .Select(t => new { Class = t.TE_Class, Teacher = t.TE_Teacher, Subject = t.TE_Subject });
            ...
            var result8 = from lesson in data.Lesson
                          where lesson.L_ClassNavigation.C_Department == "HIF"
                                && lesson.L_Day == 2
                                && lesson.L_HourNavigation.P_From.TimeOfDay > new TimeSpan(17, 0, 0)
                          select new
                          {
                              Class = lesson.L_Class,
                              Subject = lesson.L_Subject,
                              Teacher = lesson.L_Teacher,
                              Time = lesson.L_HourNavigation.P_From.ToString("H:mm")
                          };
            ...
            var latestLessonPUH = (from test in data.Lesson
                                   where test.L_TeacherNavigation.T_ID == "PUH"
                                   select test.L_Hour).Max();

            Console.WriteLine(Environment.NewLine + "RESULT9");
            Console.WriteLine(JsonSerializer.Serialize(latestLessonPUH));
            ...
            var result10 = (from teacher in data.Teacher
                            where teacher.T_ID == "SZ"
                            select new
                            {
                                ID = teacher.T_ID,
